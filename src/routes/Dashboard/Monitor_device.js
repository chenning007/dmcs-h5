...
export default class Monitor_device extends PureComponent {
  ...
  Edit_enable = value => {
    ...
    if (!value && !component_key) {
      //这样的话，是必须选中才能进行编辑，但是希望是在任何情况下都编辑
      const newequipment = this.state.equipment.map(item => ({ ...item }));
  ...
  extraContent() {
    ...
    const menu = (
      <Menu onClick={this.handleMenuClick} selectedKeys={[]}>
        <Menu.Item key="swift">开关</Menu.Item>
        <Menu.Item key="slider">滑条</Menu.Item>
        <Menu.Item key="panel">表盘</Menu.Item>
  ...
  component_control(value, key) {
    // 这部分主要是实现各种逻辑的控制，完成数据的传递过程
    const newequipment = this.state.equipment.map(item => ({ ...item }));
  ...
  modal_temp(component_key) {
    ...
    let target_position = this.getElementBykey(component_key, this.state.temporary_position);
    if (
      getFieldValue('range') !== undefined &&
      getFieldValue('measurement') !== undefined &&
      getFieldValue('node') !== undefined
    )
      if (target_equipment && target_position) {
        {
          let value = {
  ...
  // 导致modal无法及时的更新
  // value为onchange值
  modal_show(component_key) {
    // 这部分主要完成数据的展示功能，并完成数据的获取
    ...
    if (target) {
      return (
        <Modal
          ...
          cancelText="取消"
        >
          <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="量程">
          ...
          <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="端点">
  ...
  render() {
    ...
    return (
      <div>
        <Card title={this.show_title()} extra={this.extraContent()} />
        <Card
          style={{
        ...
        <div>{component_key !== null && this.modal_show(component_key)}</div>
      </div>
    );
  }
}
